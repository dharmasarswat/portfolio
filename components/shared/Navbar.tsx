...
type NavLinkType = (typeof navItems)[0];
...
export default function Navbar() {
  ...
  const handleNavLinkClick = (nav: NavLinkType) => {
    if (nav.label === 'Theme') {
      setTheme((theme) => (theme === 'dark' ? 'light' : 'dark'));
    }
    if (Boolean(nav.link)) {
      if (nav.link?.startsWith('https://')) window.open(nav.link);
      else router.push(nav.link || '/');
    }
  };
          ...
          <Fragment key={index}>
            ...
            >
              <motion.div
                ...
                animate={{
                  width: hovered === index ? 60 : 36,
                  x:
                    hovered === index
                      ? (mouseX - window.innerWidth / 2) / 30
                      : 0,
                }}
